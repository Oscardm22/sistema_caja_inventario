...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            ...
            $errors['general'] = 'Error al crear el usuario: ' . $conn->error;
        }
        $stmt->close();
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function actualizarPregunta2() {
        const pregunta1 = document.getElementById('pregunta_seguridad_1').value;
        const pregunta2Select = document.getElementById('pregunta_seguridad_2');
        
        // Habilitar todas las opciones primero
        for (let i = 0; i < pregunta2Select.options.length; i++) {
            pregunta2Select.options[i].disabled = false;
            pregunta2Select.options[i].style.display = '';
        }
        
        // Deshabilitar la pregunta seleccionada en pregunta1
        if (pregunta1) {
            for (let i = 0; i < pregunta2Select.options.length; i++) {
                if (pregunta2Select.options[i].value === pregunta1) {
                    pregunta2Select.options[i].disabled = true;
                    pregunta2Select.options[i].style.display = 'none';
                    
                    // Si la pregunta2 actual es la misma que la pregunta1, resetear
                    if (pregunta2Select.value === pregunta1) {
                        pregunta2Select.value = '';
                    }
                }
            }
        }
    ...
    // Inicializar filtro de preguntas
    actualizarPregunta2();
});
</script>
